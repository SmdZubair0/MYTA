...
SKILLS_GAP_PROMPT = promptReader.read("skills gap")

def skillGapCreationNode(ProfileState : ProfileProcessingState) -> ProfileProcessingState:
    
    llm_response = LLMClient.generate(
        prompt=SKILLS_GAP_PROMPT
        .replace("{{target_roles}}", ", ".join(ProfileState.target_roles))
        .replace("{{career_stage}}", ProfileState.career_stage)
        .replace("{{skills_summary}}", ", ".join(ProfileState.skills_summary))
        .replace("{{experience_summaries}}", ", ".join(ProfileState.experience_summaries))
    )
    ...
    return {
        "skill_gaps" : structured_data.get("skill_gaps",[])
    }
...
if __name__ == "__main__":
    ...
    # )
    # print(skillGapCreationNode(userProfile))