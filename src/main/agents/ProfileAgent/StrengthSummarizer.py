...
STRENGTHS_CREATION_PROMPT = promptReader.read("strengths")

def strengthCreationNode(ProfileState : ProfileProcessingState) -> ProfileProcessingState:
    
    llm_response = LLMClient.generate(
        prompt=STRENGTHS_CREATION_PROMPT
        .replace("{{experience_summaries}}", ", ".join(ProfileState.experience_summaries))
        .replace("{{project_summaries}}", ", ".join(ProfileState.project_summaries))
...
if __name__ == "__main__":
    ...
    # )
    # print(strengthCreationNode(userProfile))